...
- **Cost model** (empirical, per-kWp gets cheaper with size) — 3-segment model calibrated to the 2025-2026 Mirac dataset (n=80), implemented in `src/lib/calculator/cost.ts` `estimatePrice()`:
  ...
  - **Edge blending (Simon 2026-07-06)**: inside the 9-11 and 45-55 kWp bands the price is a linear interpolation between the two adjacent segment predictions, so quotes are continuous across the boundaries (the raw segments jump ~9.2M COP at 50 kWp). Outside the bands the pure segments apply unchanged. Pinned + continuity/monotonicity-tested in `cost.test.ts`.
- **Índice Mirac** (`INDICE_MIRAC = 0.0425` in `constants.ts`): annual tariff escalator used ONLY for PPA total projections (`ppaMetrics`) — the tariff indexes 4.25%/yr, the PPA price stays fixed. Not the same as `indexacion_energia` (user-configurable, drives the cash-flow engine).
...
Autonomy is in **hours** (was days — migrated). Schema: `horas_autonomia: z.number().min(1).max(168)`. Default/fallback is **8 h** (night coverage; was 48 — changed 2026-07-06 in defaults.ts, index.ts and the MCP schema). Proposals saved with an explicit 48 keep it; only the fallback changed.

All battery display sites have defensive guards (`typeof horas === 'number'`) so old proposals saved before the rename don't crash.

**Deliberate simplification (Simon 2026-07-06)**: the battery model does NOT model round-trip efficiency losses in the savings loop (`eficiencia` is stored/displayed but doesn't reduce `autoConsumo`). Keep it that way unless Simon asks — the conservatism elsewhere (min(gen, consumo) cap) covers it commercially.
...
- **Editing model (2026-07-06)**: committed roofs are selectable (click the polygon or its sidebar row) and the selected roof is an **editable `<Polygon>`** — drag vertices/midpoints; the path is read back on `onMouseUp` from a `polygonRefs` Map (populated in `onLoad`, never read during render) and committed only if changed, avoiding the `set_at` feedback loop when React echoes the `paths` prop. The selected roof gets a sidebar card with per-area **rotation** (−90..90°) and **row-gap** (0-2 m) sliders and a "Rellenar paneles" button; panels of the selected roof are clickable to delete individually. **Re-pack rule**: any vertex/rotation/gap/orientation change regenerates the area's panels via `repack()` (`packing.ts`) and discards manual deletions (stated in the hint copy). Vertex edits re-pack only already-filled roofs; orientation is design-level and re-packs all filled roofs. There is no global row-gap slider — new roofs take `defaultRowGap(tipoCubierta)`.
...
- `PATCH /api/share` updates `c` (client short-keys) and/or `d` (DocuSeal state) on the stored payload. The `docusealPatch` is a `{ submission_id, submitter_slug }` claim verified against the DocuSeal API before anything is stored (uniform 404 on mismatch) — this is how signing on `/s/<id>` survives reloads.
- `GET /api/share?id=X` counts a client view (`share_views:<id>` / `share_last_view:<id>`); append `no_track=1` for owner-side fetches. `?stats=<ids>` returns batched counters without incrementing. `DELETE /api/share?id=X` revokes a link.
- Proposals carry `share_id`/`shared_at` once shared (set by `ShareDialog`, which also auto-advances draft → sent); `s:<share_id>` is the DocuSeal external_id on shared signing and one of the two keys the Drive webhook mapping is registered under.
...
- **Tools**:
  - `quote_solar_system` (`readOnlyHint`) — friendly small input (`consumo_mensual_kwh` + `ciudad` minimum; optional tarifa, clima, cubierta, modo_conexion, baterias, financiamiento EA, beneficios tributarios). Location: `ciudad` enum, or `lat`/`lon` (any Colombian municipality — resolves real radiation via `resolveHsp()` → `src/lib/pvgis.ts` fetchPVGIS with climate-estimate fallback), or `hsp_personalizado` (12 monthly values; highest precedence). `buildStores()` maps the friendly args to the web app's store shapes (technical/project/advanced) including `project.lat/lon/hsp_mensual_pvgis`, then `summarize()` runs `cotizacion(buildInputFromStore(...))`. Returns a Spanish markdown summary + `structuredContent` (headline numbers, `roi_proyecto_porcentaje`, `co2_ton_anio`/`co2_ton_vida_util`, `inversores[]`, and a `supuestos{}` block: hsp promedio/fuente/fallback, PR, degradación, indexación, horizonte, excedentes, descuento).
  - `estimate_price` (`readOnlyHint`) — quick CAPEX from `kwp` via `estimatePrice()`; optional `cubierta` (teja surcharge) and `bateria_capacidad_kwh` (400k COP/kWh default).
  - `list_inverters` (`readOnlyHint`) — the `INVERTER_DATABASE` catalog (marca/tipo/modelos+kW), optional `marca` filter; tells the agent how to use `inversor_marca`/`inversor_potencia_kw`.
  ...
  - `get_quotation` / `update_quotation` (`src/lib/mcp/manage-link.ts`) — read or regenerate an existing `share:<id>` without orphaning the sent link (see Recent context #44 for semantics and guardrails).
  ...
  - **Error handling**: every tool handler is wrapped by the `tool()` helper in the route — failures return a Spanish `isError` result, never a raw JSON-RPC error.
  - **Rate limit**: 60/min per IP in `guarded()` (skipped when Upstash is unconfigured).
...
- `DOCUSEAL_WEBHOOK_SECRET` — **required in production** (since 2026-07-06 the webhook fails closed when `NODE_ENV === 'production'` and the var is unset; local dev stays open). When set, `/api/docuseal/webhook` validates the `X-Docuseal-Signature` header.
...
36. **Share 400 fixed — non-finite numbers serialize to JSON `null` and the `/api/share` zod gate rejects `null`.** Symptom: generating a link from the virtual quotation returned `POST /api/share 400 ("Formato de propuesta inválido")`. Root cause (reproduced against the live prod endpoint): a non-finite technical number — most reachably `ancho_m`/`alto_m`, since react-hook-form `valueAsNumber` yields `NaN` for an emptied field and `?? default` doesn't catch `NaN` — becomes `null` under `JSON.stringify`, and the gate types these as `z.number()` / `z.number().optional()` (optional accepts `undefined`, **not** `null`). Fix: `toPayload()` (`src/lib/share.ts`) now sanitizes at the serialization boundary — `finiteOr()` coerces `co/pw/fs/an/al` to defaults, `sanitizeRoofDesign()` zeroes any non-finite roof geometry number (area/rotation/row_gap/vertices/panels), and a malformed `hsp` (length ≠ 12 or any NaN element) collapses to `null`. Because `create_quotation_link` reuses `toPayload()`, the MCP link path is covered too. Also made the designer's `anchoM/altoM` derivation finite-safe in `step-technical.tsx` (a NaN there silently packed 0 panels). Regression test: `src/lib/__tests__/share-sanitize.test.ts` mirrors the route's gate schema and asserts the JSON-round-tripped `toPayload` output validates for NaN/Infinity/malformed-hsp inputs (5 tests). Typecheck + lint (0 errors) + 55 tests + build all green. **Gate-vs-source note**: the `/api/share` gate is intentionally a security gate (size/length caps), not business validation — keep numeric sanitization at `toPayload`, don't loosen the gate's `z.number()` types.
37. **MCP: manual price + PPA support added** (`quoteInputShape` in `src/lib/mcp/quote.ts`): `precio_manual_cop` (optional, maps to `advanced.precio_manual` — the engine already honors it at `index.ts:192`) and `ppa_opciones` (array of `{precio_kwh, duracion_anios}`, maps to `advanced.ppa`; enables PPA when non-empty — deepMerge replaces arrays wholesale, so an empty array correctly clears the two default options). `summarize()` prints a "Precio manual aplicado" line and a PPA block via `ppaMetrics()` from `derived.ts`, and adds `precio_manual_aplicado` + `ppa_opciones` to `structuredContent`. Both flow through `create_quotation_link` unchanged (advanced is stored wholesale in the share payload, so the `/s/<id>` page shows the manual price and the PPA section live). Gotcha caught in smoke test: `formatCOP` ceils to the nearest 100 — per-kWh prices must pass `round=false` (550 printed as $600). Typecheck + lint (0 errors) + 55 tests + build green.
38. **MCP: string-number coercion + inverter selection** (`src/lib/mcp/quote.ts`).
    - **Coercion fix**: MCP clients (Cowork) serialize numeric args as JSON strings (e.g. `"32000000"`), which `z.number()` rejects even though the published `inputSchema` correctly declares `type: number`. All numeric inputs switched to `z.coerce.number()` so string- or number-encoded values both validate. (The Cowork-side theory that the schema "didn't declare the field" was wrong — verified via `tools/list`; the field was always declared as number. Root cause was strict server-side type checking of a string-encoded value.)
    - **Inverter selection** (mirrors the panel selection): four optional inputs — `inversor_marca`, `inversor_modelo`, `inversor_potencia_kw`, `inversor_cantidad` — mapped by the `inverterFields()` helper to the store's four inverter fields (`marca_inversor`/`marca_inversor_custom`/`modelo_inversor`/`override_inversores`, all on `AdvancedData`, stored wholesale in the share payload so no `share.ts` change). Logic: a known brand (in `INVERTER_DATABASE`) with no explicit model resolves its catalog model (e.g. `Deye` + `inversor_potencia_kw: 5` → `SUN-5K-SG04LP1`); an explicit free-text model or an unknown brand routes through the `'Otro'` custom path so the proposal shows exactly what was passed; `inversor_potencia_kw` becomes a single `override_inversores` row (else the calculator auto-picks by size). The `summarize()` inverters line now prints `marca modelo (kW)`. Both flow through `create_quotation_link` unchanged. Typecheck + lint (0 errors) + 55 tests + build green.
39. **2026-07-06 five-agent audit + upgrade roadmap** — parallel subagent audit (security, calculator, product/UX, code quality, integrations); no code changed. Full prioritized roadmap: `docs/superpowers/plans/2026-07-06-upgrade-roadmap.md`. Headline findings (all except (e) and the [Simon]-gated pieces were fixed the same day — see items 40-45): (a) `cost-comparison-section.tsx` has an off-by-one into `flujo_caja` AND a parallel hand-rolled savings loop (hardcoded 2%-CAPEX O&M, no surplus/tax/cuotas) — the "Costo de No Hacer Nada" chart disagrees with the engine; (b) 0%-interest financing skips the cuota entirely (`index.ts:203` `tasaInteresCredito > 0` guard) = free money; (c) payback prints 0 años at 100% financing (`-0` year-0 flow); (d) `estimatePrice` throws for kWp ≤ 0, reachable from shared/imported payloads (crashes `/s/[id]`); (e) cost model has a +9.2M COP discontinuity at the 50 kWp segment boundary (needs Simon — coefficients are calibrated); (f) **the shared-link signing loop is broken end-to-end**: `fromPayload` hardcodes `id: 'shared'` / create-link uses `'mcp'`, so the DocuSeal webhook's Drive-mapping lookup never matches, PATCH /api/share can't persist `docuseal` state (clientPatch-only), and a reload after signing mints a duplicate submission; (g) Drive re-sync always creates a brand-new `FVyyNNN` folder tree; (h) DocuSeal webhook accepts unsigned requests when `DOCUSEAL_WEBHOOK_SECRET` is unset and returns 200 on Drive-upload failure (DocuSeal only retries non-2xx); (i) MCP handler, `/api/pvgis`, and `scan-bill` have no rate limiting; MCP booleans lack the string-coercion fix that numbers got in item 38. June-2026 hardening verified intact; `tsc`/lint/55 tests/`npm audit` all clean. Decisions parked for Simon: cost-curve blending, battery round-trip-efficiency modeling, `DOCUSEAL_WEBHOOK_SECRET` in Vercel, PPA indexation footnote, dead `medidor_inteligente` toggle.
40. **Roadmap Phase 0 implemented (branch `fix/phase0-correctness`)** — the six non-[Simon] correctness bugs from item 39: (1) `cost-comparison-section.tsx` now reads the engine's `flujo_caja[i].flujo_acumulado_cop` directly (off-by-one + parallel hand-rolled loop deleted — the chart's break-even now always matches `payback_anios`); (2) 0%-interest loans charge cuotas (`pmt` handles rate 0; the `tasaInteresCredito > 0` guard is gone); (3) payback only reports a crossing after the cumulative flow has been negative — 100% financing no longer prints 0 años unless the flow genuinely never dips; (4) panel count clamps to ≥ 2 (finite) so `consumo=0`/`override_paneles=0` payloads can't reach `estimatePrice(kwp<=0)` and crash `/s/[id]`; (5) sub-3 kWp systems floor at the smallest 3 kW inverter instead of an empty combo; (6) duplicate `override_inversores` rows merge quantities. 7 new golden tests; zero existing snapshot diffs (no previously quoted number changed).
41. **Roadmap Phase 1 — shared-link signing loop closed (branch `feat/phase1-signing-loop`)** — finding (f) of item 39 fixed end-to-end. `fromPayload(payload, shareId)` now derives a real `id: s:<shareId>` + `share_id` (constants `'shared'`/`'mcp'` gone), so the DocuSeal `external_id` is unique per link and the webhook's Drive-mapping lookup matches. `PATCH /api/share` accepts a `docusealPatch { submission_id, submitter_slug }` — verified against the DocuSeal API server-side (uniform 404 on mismatch; nothing client-supplied is stored) — and `/s/[id]` persists signing state on update, so a reload no longer mints a duplicate submission. Payloads with a completed signature load as `status: 'accepted'`. `ShareDialog` records `share_id`/`shared_at` on the proposal and auto-advances draft → sent; the local virtual page syncs signing state back from the share payload (with `no_track=1`). Drive re-sync reuses the persisted `drive_upload_folder_id` (new `QuotationData` field, along with `share_id`/`shared_at`) instead of minting a new FVyyNNN tree, and registers the webhook mapping under both the local id and `s:<share_id>`. The webhook returns 500 on Drive-upload failure or null link (DocuSeal retries non-2xx); all DocuSeal fetches carry 15s timeouts; `uploadSignedContractToDrive` no longer reports success with a null link.
42. **Roadmap Phase 2 — security batch (branch `feat/phase2-security`)** — `rateLimit()` is now atomic (`MULTI` INCR + EXPIRE NX; no more TTL-less-counter crash window). Per-IP rate limits on the previously open surfaces: MCP handler (60/min in `guarded()`), `/api/pvgis` (30/min; lat/lon rounded to 3 decimals so the 24h fetch cache hits) and the bill-scanner server action (10/5min, IP via `next/headers`); all skip when Upstash is unconfigured (local dev). MCP input bounds (`ppa_opciones` ≤ 20 with capped fields, tasa EA ≤ 1, plazo ≤ 30 años, `.max()` on free-text, numeric ceilings) plus a 4.5MB serialized-payload check in `create-link.ts` (same cap as POST /api/share). `docuseal-sign-dialog` only embeds `https://*.docuseal.com` URLs (embed_src can come from untrusted storage). **Deferred pending Simon:** webhook fail-closed when `DOCUSEAL_WEBHOOK_SECRET` is unset — would break prod signing if the var is missing in Vercel; confirm it first.
43. **Roadmap Phase 3 — sales loop (branch `feat/phase3-sales-loop`)** — view tracking: `GET /api/share` INCRs `share_views:<id>` + sets `share_last_view:<id>` (90d TTL, best-effort); `GET /api/share?stats=<ids>` (≤60, comma-separated) returns batched counters without incrementing; owner-side fetches pass `no_track=1` so Simon's own opens don't count. `/propuestas` gained pipeline value cards (enviadas/aceptadas/conversión), "Visto N veces · hace X días" on shared cards, and a "Seguimiento pendiente" banner (status sent, `shared_at` ≥ 7 días, sin firma → one-click `wa.me` follow-up). `/propuestas/[id]` gained an "Enlace compartido" card: link + copy, view stats, sent date, Regenerar (new id), Revocar (`DELETE /api/share`, clears stats keys too). New `share.ts` helpers: `fetchShareStats`, `revokeShareLink`; `generateShareUrl`/`generateMultiShareUrl` now return `{ url, id }`. Item 23 (first-view email/webhook notification) parked — needs a provider decision from Simon.
44. **Roadmap Phases 4a + 5 — store backfill + MCP parity (branch `feat/phase4-5-storage-mcp`)** — `proposals-store` now has a rehydrate `merge` that deep-merges every persisted proposal against initial defaults (the defense quotation-store already had). MCP: booleans accept `"true"/"false"` strings via the `boolInput()` preprocess (published schema stays `type: boolean`; deliberately NOT `z.coerce.boolean()`, which treats `"false"` as true); battery auto-enables when `bateria_capacidad_kwh > 0`; panel selection args (`panel_marca`/`panel_modelo`/`numero_paneles` → `override_paneles`); financial + battery detail args (`horizonte_anios`, `tasa_descuento`, `indexacion_energia`, `porcentaje_mantenimiento`, `demora_6_meses`, `bateria_profundidad_descarga`, `bateria_costo_kwh`). New tools in `src/lib/mcp/manage-link.ts`: `get_quotation` (reads a `share:<id>`, reports numbers + signing state; handles multi-version) and `update_quotation` (regenerates the payload WITHOUT changing the sent `/s/<id>` link; preserves client identity, lat/lon + PVGIS data, signing state and remaining TTL via `keepTtl`+`xx`; refuses signed or multi-version links; **full-regeneration semantics** — unsent params reset to defaults, warned in the tool description). All five tool handlers wrapped in try/catch → Spanish `isError` results. Smoke-tested live against `/api/mcp` (string booleans validate, schema publishes `type: boolean`, error path returns Spanish isError). **Deferred:** item 28 (arbitrary lat/lon + PVGIS HSP in MCP — extract the `/api/pvgis` logic into a lib first) and items 24b/25 (drop persisted `results`, record-per-proposal layout, Upstash-primary — storage re-architecture on live data, needs Simon).
45. **Phase 7 cleanup (roadmap items 38-40, branch `chore/phase7-cleanup`)** — dead-code removal + cheap test debt; `medidor_inteligente` untouched (still needs Simon). Deleted 10 zero-importer `src/components/ui/` files (calendar, checkbox, dropdown-menu, navigation-menu + its navigation-menu-variants, popover, scroll-area, select, sheet, slider) and uninstalled `date-fns` + `react-day-picker` (only calendar.tsx used them). Removed the legacy re-export block in `quotation-store.ts` (initial*/deepMerge); the one consumer (`step-advanced.tsx`) now imports from `@/lib/defaults`. `derived.ts` O&M now reads `DEFAULT_PARAMS.porcentaje_om_anual` instead of a hardcoded 0.02 (same value). PDF plazo-in-years now uses `Math.round` like the web financing card (was `Math.floor`). `convertWithMarkItDown` (`scan-bill.ts`) logs a `console.warn` on non-OK/error responses so a dead MarkItDown service is visible in logs. New `src/lib/calculator/__tests__/cost.test.ts` (12 tests): pins `estimatePrice` at 9.99/10/50/50.01 kWp (documents the ~9.2M COP 50-kWp discontinuity as a deliberate-diff guard), throws-on-kwp<=0, `recomendarInversor` sanity at 5/12/60/120 kWp, carbon positivity + linearity. **Roof packing last-row bound NOT changed**: the audit claim is inverted — the loop bound `cy + cellH/2 <= maxY` is already *stricter* than the suggested `cy + h/2 <= maxY`, so no wasted iterations exist; "fixing" it would loosen the bound and can add a last row on losa roofs (rowGap 0.7 m), i.e. change packed counts. Needs Simon to decide whether that extra row is wanted. Gates: tsc, lint (0 errors / 19 known warnings), 77 tests, build all green.
46. **2026-07-06 MCP backlog + pending [Simon] decisions implemented (branch `feat/mcp-backlog`)** — Simon tested the MCP tools in production and wrote a 10-item backlog (`MCP_Calculadora_Solar_Backlog_Mejoras.md` in Drive); backlog items #1-core/#2/#3/#9 had already landed on the stacked branches and only needed deploy. Decisions confirmed via AskUserQuestion and implemented:
    - **Battery autonomy default 48h → 8h** (defaults.ts, `index.ts` fallback, MCP schema): 8h covers the night; 48h was oversizing/overpricing every auto-sized battery quote.
    - **Dual ROI**: new `roi_proyecto_porcentaje` on `CalculationResults` (unlevered — flows before cuotas ÷ full CAPEX; equals `roi_porcentaje` without credit). Financed quotes show "ROI Proyecto" + "ROI Inversionista" in the web financial card and both in the MCP summary/structured. Old persisted results lack the field — access via `?? r.roi_porcentaje`.
    - **PPA totals indexed at Índice Mirac** (`INDICE_MIRAC = 0.0425`, constants.ts): `ppaMetrics().ahorroTotal` escalates the tariff 4.25%/yr with the PPA price fixed (per-year clamp at 0; fractional last year pro-rated). Footnotes added in `ppa-section.tsx` and the PDF PPA page. `ahorroAnual` stays year-1.
    - **Cost-curve blending**: `estimatePrice` linearly interpolates between adjacent segments in the 9-11 / 45-55 kWp bands (kills the ~9.2M COP jump at 50 kWp — audit item (e) closed). `cost.test.ts` re-pinned: pure-segment values outside bands unchanged; in-band pins + continuity (<100k per 0.01 kWp) + monotonicity asserts. Golden snapshots untouched (base cases sit outside the bands).
    - **MCP geographic coverage**: PVGIS logic extracted to `src/lib/pvgis.ts` (route is a thin wrapper); `quote_solar_system`/`create_quotation_link`/`update_quotation` accept `lat`/`lon` (Colombia bounds, PVGIS with `estimada` fallback labeling) and `hsp_personalizado` (12 values, top precedence) via `resolveHsp()`; resolved coords+HSP are stored on the project so `/s/<id>` shows the map and recomputes identically. `update_quotation` keeps the stored location only when the update sends none.
    - **MCP structured output**: `supuestos{}` (hsp_promedio_dia, hsp_fuente ciudad|pvgis|estimada|personalizada, fallback_radiacion, PR, degradación, indexación, horizonte, excedentes, descuento), `inversores[]`, `co2_ton_anio`/`co2_ton_vida_util`, `roi_proyecto_porcentaje`.
    - **New `list_inverters` tool** (6th tool) — catalog with optional brand filter; `estimate_price` gained `cubierta` + `bateria_capacidad_kwh`.
    - **Webhook fail-closed in production** when `DOCUSEAL_WEBHOOK_SECRET` is unset (Simon confirmed the var is/will be set in Vercel — set it BEFORE deploying this).
    - **`medidor_inteligente` removed** (types/schema/defaults/form; it never affected math). `medidor_bidireccional` (1.3M COP adder) stays.
    - **PDF ficha técnica**: panel line prints marca/modelo when set; inverter line prints `cantidad× marca modelo` (kW suffix only when the model text lacks "kw"), matching `project-details-section.tsx`.
    - **Left as-is per Simon**: battery round-trip efficiency (deliberate simplification, documented under "Battery sizing"), roof-packing last-row bound (stays conservative).
    - Gates: tsc, lint (0 errors / 19 known warnings), 82 tests, build green; MCP smoke test against local dev verified all 6 tools, PVGIS lat/lon (La Ceja → hsp_fuente pvgis), battery 8h auto-size, dual ROI and indexed PPA totals. **Merged the full stack to main; Simon pushes to deploy.**
47. **Virtual quotation content additions (branch `feat/virtual-aspectos-validez`)** — three client-facing additions, all static copy (no schema/store/share/calculator changes, so old proposals and `/s/` links need nothing):
    - **`important-aspects-section.tsx`** (new, collapsed by default): the PDF's `aspectos_a.jpg`/`aspectos_b.jpg` content transcribed to real text — "Condiciones del sistema" bullets, "Garantías" as 3 visual cards (Simon-corrected values: 1.5 años instalación / 15 paneles / 5 inversores, replacing the warranty bullet), "Entregables" list, and "Incentivos tributarios" (condensed `aspectos_b` text, gated on `advanced.beneficios_tributarios`). **Battery-aware**: when `results.bateria?.habilitada`, the grid-only bullet swaps for a battery-backup bullet. The "on grid, no está preparado para incluir baterías" bullet was **removed entirely** per Simon (hybrid inverters accept batteries). (The PDF background JPGs still carry the old garantías + no-baterías text — known limitation; fixing needs new images.)
    - **`faq-section.tsx`** (new, collapsed by default): 6 static Q&A items (mantenimiento — anual, precio incluido en la propuesta, per Simon; días nublados, excedentes, venta del inmueble, duración instalación, granizo/viento) with per-item toggles.
    - **Offer validity 30 días**: `ofertaValidezHasta()` + `formatFechaLarga()` + `OFERTA_VALIDEZ_DIAS` in `formatting.ts` (invalid fecha → null → static fallback line). Shown under the header (`ValidityNotice` in `virtual-quotation.tsx`) and above the CTA buttons (`validezHasta` prop on `CallToAction`); expired offers show an amber "venció el X, contáctanos" notice but do NOT block DocuSeal signing (Simon's choice). PDF cover gained a small "Oferta válida por 30 días" line under the fecha. Gotcha: React Compiler purity rules reject bare `Date.now()` in render — both display sites capture it via a `useState` initializer.
    - Sections render between `ProjectDetailsSection` and `NotesSection`; collapsible pattern copied from `NotesSection` (custom button + chevron, deliberately no Radix accordion). Gates: tsc, lint (0 errors / 19 known warnings), 82 tests, build green.
    - **`payment-schedule-section.tsx`** (follow-up, same session): "Esquema de Pagos" 3-step visual flow — 50% Anticipo → 45% Fin de instalación → 5% Legalización ante el OR, amounts derived from live `results.costo_total_cop`. Renders right after `PricingTable`, **hidden when `advanced.financiamiento?.habilitado`** (a financed client pays anticipo + cuotas; the financing card covers that). Web-only for now — adding it to the PDF needs a placement decision on the fixed background layouts.
48. **Roof designer "editing power" upgrade (branch `feat/roof-designer-editing`)** — committed roofs are no longer immutable (see the "Editing model" bullet under the Roof designer domain rule). New: roof selection (map click or sidebar row), editable `<Polygon>` vertex dragging with `onMouseUp` path read-back via a `polygonRefs` Map, per-area rotation + row-gap sliders with live re-pack, per-panel click-to-delete on the selected roof + "Rellenar paneles", orientation change re-packs all filled roofs, global row-gap slider removed (new roofs take `defaultRowGap(tipoCubierta)`). New pure helper `repack()` in `packing.ts` (+5 tests: rotation determinism/containment, 90°-rotation ≈ orientation-swap at matching gaps, repack composition, degenerate polygon). No type/schema/share/snapshot/step-technical changes — `rotation_deg`/`row_gap_m` already existed per area. Panel polygons keyed by center coords (not array index). Gates: tsc, lint (0 errors / 19 known warnings), 86 tests, build green. Preview-browser smoke verified the state loop end-to-end (auto-fill 78 panels on a 15×15 m seed, rotation 45° → 57, gap 1 m → 40, restore → 78, Aplicar → `override_paneles=78` + 265 KB snapshot persisted, designer round-trips) — but the interactive Maps engine doesn't bootstrap in that headless environment, so **map-gesture paths (vertex drag, roof click-select, panel click-delete, Google's built-in undo chip after a drag) still need a real-browser smoke by Simon**. Also found (not fixed, chip spawned): pre-existing hydration race on direct load of `/cotizacion?edit=<id>` — infinite spinner because the init effect runs before the proposals store rehydrates and a ref guard blocks the retry; client-side nav from `/propuestas` is unaffected.
49. **Hydration race fixed on `/cotizacion?edit=<id>` direct loads** (`src/app/cotizacion/page-client.tsx`): the init effect could run before the proposals store rehydrated, call `reset()`, and the `lastEditId` ref guard then blocked the re-run when the proposal appeared — `ready` (`editingId === editId`) never held and the loading spinner spun forever (client-side nav from `/propuestas` was unaffected, stores already hydrated). The guard is now transition-aware: initializing without the proposal marks the bare `editId` (pending); when `proposalToEdit` arrives the effect upgrades to `loadProposal` and marks `<editId>:loaded`; once loaded it never re-loads, so a later `proposalToEdit` identity change (e.g. `updateProposal`) can't clobber an in-progress edit. Browser-verified all three direct-load paths (valid id → Editar with data, bogus id → Nueva, no param → Nueva/empty form); tsc + lint (0 errors) + 82 tests green.
50. **Precio sin discriminar por sistema, IVA visible (2026-08-04)** — Simon: las baterías SÍ llevan IVA, y el desglose anterior (FV sin IVA / Almacenamiento sin IVA) lo insinuaba al revés. Se eliminó la separación FV vs almacenamiento en las dos superficies cliente; el IVA **sí se sigue mostrando**. `pricing-table.tsx` (virtual) y la página 7 del PDF (`proposal-pdf.tsx`) ahora muestran: Subtotal (sin IVA) → IVA → Inversión Total → O&M Anual. `ivaBreakdown()` en `derived.ts` queda intacto. El total no cambia: el motor ya sumaba FV + batería con IVA incluido. El fondo `9.jpg` rotula "Valor sistema FV sin IVA" para la primera línea — ahora esa cifra es el subtotal completo (FV + batería), así que conviene regenerar la imagen con la etiqueta "Subtotal (sin IVA)".
51. **Drive: carpeta raíz de proyectos equivocada (2026-08-24)** — `PARENT_FOLDER_ID` apuntaba a `1lsky0585MIO6iuZGv1jYU0ccHpArNVR8` = `Mirac/Proyectos_FV`, una carpeta vieja que sigue existiendo (por eso el sync "funcionaba" sin error) pero que ya no es donde vive la operación: solo tenía 8 carpetas FV26001-FV26008. Los proyectos reales están en **`05_Proyectos_FV`** = `1egY0farrALY_APD7kNjGcKWwLfdpfkjD` (misma unidad compartida `Mirac`), con la serie hasta FV26187. Efecto secundario del ID viejo: el consecutivo se calculaba sobre esas 8 carpetas, así que reasignaba números ya usados (una prueba creó `FV26008 - dsadasd`). Solo cambia el env var — no hubo cambios de código. **Hay que actualizar `PARENT_FOLDER_ID` también en Vercel** (y reiniciar `npm run dev` en local) o se siguen creando en la carpeta vieja.
