...
50. **Precio sin discriminar (2026-08-04)** — Simon: las baterías SÍ llevan IVA, y el desglose lo insinuaba al revés. Se retiró la discriminación de precios en las dos superficies cliente: `pricing-table.tsx` (virtual) ahora muestra solo "Inversión Total (IVA incluido)" + O&M Anual (se eliminaron las líneas FV sin IVA / Almacenamiento sin IVA / IVA), y la página 7 del PDF (`proposal-pdf.tsx`) ya no imprime los valores de "FV sin IVA" ni "IVA" — solo total + O&M. `ivaBreakdown()` en `derived.ts` queda intacto (el PDF sigue usando `costoSinIVA` para la línea de deducibles). El total no cambia: el motor ya sumaba FV + batería con IVA incluido. **Limitación conocida**: el fondo `9.jpg` todavía trae impresas las etiquetas "Valor sistema FV sin IVA" e "IVA", que ahora quedan sin cifra — hay que regenerar la imagen de fondo.
