...
47. **Virtual quotation content additions (branch `feat/virtual-aspectos-validez`)** — three client-facing additions, all static copy (no schema/store/share/calculator changes, so old proposals and `/s/` links need nothing):
    - **`important-aspects-section.tsx`** (new, collapsed by default): the PDF's `aspectos_a.jpg`/`aspectos_b.jpg` content transcribed to real text — "Condiciones del sistema" bullets, "Garantías" as 3 visual cards (3 años instalación / 12 paneles / 5 inversores, replacing the warranty bullet), "Entregables" list, and "Incentivos tributarios" (condensed `aspectos_b` text, gated on `advanced.beneficios_tributarios`). **Battery-aware**: when `results.bateria?.habilitada`, the two on-grid/"no baterías" bullets swap for a single battery-backup bullet. (The PDF background JPGs still say "no está preparado para incluir baterías" on battery quotes — known limitation; fixing needs new images.)
    - **`faq-section.tsx`** (new, collapsed by default): 6 static Q&A items (mantenimiento, días nublados, excedentes, venta del inmueble, duración instalación, granizo/viento) with per-item toggles.
