...
51. **Drive: carpeta raíz de proyectos movida (2026-08-24)** — el `PARENT_FOLDER_ID` apuntaba a una carpeta que ya no existe (`files.get` devolvía "Requested entity was not found"), así que `prepararCarpetaDrive` fallaba al crear el árbol `FVyyNNN` y la propuesta no se guardaba. Nuevo destino: **`05_Proyectos_FV`** = `1egY0farrALY_APD7kNjGcKWwLfdpfkjD` (verificado con el refresh token del app: `canAddChildren: true`, ya contiene FV26xxx así que el consecutivo sigue correcto). Solo cambia el env var — no hubo cambios de código. **Hay que actualizar `PARENT_FOLDER_ID` también en Vercel** o producción sigue rota.
